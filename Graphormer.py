...
class CentralityEncoding(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.LongTensor) -> torch.Tensor:
        ...
        in_degree = self.decrease_to_max_value(x=in_degree, max_value=self.max_in_degree - 1)

        out_degree = torch_geometric.utils.degree(index=edge_index[0], num_nodes=num_nodes).long()
        out_degree = self.decrease_to_max_value(x=out_degree, max_value=self.max_out_degree - 1)
...
model = Graphormer(num_layer=1,
                   ...
                   max_in_degree=1,
                   max_out_degree=1,
                   max_path_distance=19,
                   num_graph=1)

node_feature = torch.randn(8, 32)  # 8 nodes, 32 feature dimension
